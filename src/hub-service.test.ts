...
describe('testHubPage', () => {
  it('uses only HTTP when renderMode is http', async () => {
    const httpCrawl = vi.fn<CrawlPage>().mockResolvedValue([item(1)]);
    const playwrightCrawl = vi.fn<CrawlPage>();

    const result = await testHubPage('https://example.com/news', 'http', {
      httpCrawl,
      playwrightCrawl,
    });
  ...
  it('falls back to Playwright when auto HTTP results are too few', async () => {
    const httpCrawl = vi.fn<CrawlPage>().mockResolvedValue([item(1)]);
    const playwrightCrawl = vi.fn<CrawlPage>().mockResolvedValue([item(2), item(3)]);

    const result = await testHubPage('https://example.com/news', 'auto', {
      httpCrawl,
      playwrightCrawl,
    });
  ...
  it('falls back to Playwright when the auto HTTP request fails', async () => {
    ...
    const playwrightCrawl = vi.fn<CrawlPage>().mockResolvedValue([item(1)]);
  ...
  it('forces Playwright without making an HTTP request', async () => {
    ...
    const result = await testHubPage('https://example.com/news', 'playwright', {
      httpCrawl,
      playwrightCrawl,
    });
