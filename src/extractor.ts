...
function extractNearbyDate($: CheerioAPI, anchor: Cheerio<AnyNode>): string | null {
  const container = anchor.closest('article, li, .card, .item, .news, .result, div').first();
...
export function extractCandidateItems(html: string, pageUrl: string): HubItem[] {
  ...
  const candidates: Array<HubItem & { sameHost: boolean }> = [];

  $('a[href]').each((_, element) => {
    const anchor = $(element);
    const title = normalizeText(anchor.text() || anchor.attr('aria-label') || '');
    if (title.length < MIN_TITLE_LENGTH || unwantedTextPattern.test(title)) return;
  ...
  return candidates
    .sort((left, right) => Number(right.sameHost) - Number(left.sameHost))
    .slice(0, MAX_ITEMS)
    .map(({ sameHost: _sameHost, ...item }) => item);
}
